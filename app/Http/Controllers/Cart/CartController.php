...
use App\Http\Controllers\Model\Master_shipment; // ongkir

// LOCATION
use App\Http\Controllers\Model\Master_provinces; // Profinsis
...
{ 
    public function test_email($id_order, $email,$name,$grand_total){
        ...
        Mail::send('emails.mail', $data, function($message) use ($to_name, $to_email) {
            ...
            $message->from('noreply@fibercreme.com','Fibercreme Onlinestore');
        });
    }

    public function coba_email(){
        $transaction = Transaction::getTrans()->where('id_order','=','1554351785')->first();
        $detail = Detail::getDetail()->where('id_order','=','1554351785');
        $customer = Master_customer::all()->where('id','=','243')->first();

        $to_name = 'Jhon Doe';
        $to_email = 'lutfi.febrianto@gmail.com';
        $data = array('name'=>$to_name, "body" => "Total pembayaran anda = Rp. 50.000", 'detail'=>$detail, 'transaction'=>$transaction);
    ...
    public function noreload(){
     ...
     public function checkout(){
        ...
        $provinces = Master_provinces::all()->where('is_active','=','1');
        if (Cart::content() == '[]'){
    ...
    public function province_ajax(){ 
        ...
        $regency = Master_regencies::where('province_id','=',$province_id)->where('is_active','=','1')->get();
    ...
    public function regency_ajax(){ 
        ...
        $district = Master_districts::where('regency_id','=',$regency_id)->where('is_active','=','1')->get();
    ...
    public function district_ajax(){ 
        ...
        $village = Master_villages::where('district_id','=',$district_id)->where('is_active','=','1')->get();
        return Response::json($village);
    }

    public function ongkir_ajax(){ 

        $zone_id = Input::get('zone_id');
        $ongkir = Master_shipment::where('to_zone','=',$zone_id)->where('is_active','=',1)->get();
    ...
    public function insert_order(Request $request){
        if (Cart::content() == '[]'){
            ...
            // table biodata
            $village_explode = explode(";", $request->village);
                $village_explode[0]; 
                $village = $village_explode[0]; // village

                $village_explode[1]; // zone 
                $zone = $village_explode[1]; // zone
            ...
            $berat = 0;
            foreach(Cart::content() as $detailberat){
                $berat = $berat+($detailberat->options->weight * $detailberat->qty);
            }
            $berat = ceil($berat / 1000);
            

            $cek_ongkir = Master_shipment::where('to_zone','=',$zone)->where('is_active','=',1)->first();
            $ongkir = $cek_ongkir->price * $berat;
            $order = new Transaction;
                ...
                $coupon = Master_coupon::where('coupon_code','=',strtoupper($request->coupon_code))->where('is_active','=',1)->first();
                    ...
                    // Cek coupon
                    if ($coupon) {
                        if ($date_str >= strtotime(date($coupon->start_date)) || $coupon->start_date == null) {

                            if ($date_str <= strtotime(date($coupon->end_date)) || $coupon->end_date == null) {
                                 // insert row
                                $order->id_coupon = $request->coupon_code;

                                if ($coupon->quota_used == null || $coupon->quota_used == 0) {
                                    $quota_used = 0;
                                }else{
                                    $quota_used = $coupon->quota_used;
                                }

                                $quota_used = $quota_used + 1;
                                $coupon->quota_used = $quota_used;

                                // disable coupon 
                                if ($quota_used >= $coupon->quota) {
                                    $coupon->is_active = 2;
                                }
            ...
            // Create Order =================================================================
                ...
                $data = [
                    ...
                    'parcel_job' => array(
                        ...
                        'cash_on_delivery' => $transaction->grand_total,
    ...
    public function check(Request $id){
                ...
                if ($date_str >= strtotime(date($check_coupon->start_date)) || $check_coupon->start_date == null) {

                    if ($date_str <= strtotime(date($check_coupon->end_date)) || $check_coupon->end_date == null) {
                        $ongkir = $id->ongkir;
    ...
    public function total_pembayaran(Request $request){
    	$ongkir = $request->ongkir;
        $total_pembayaran = (Cart::subtotal(null,null,'') + $ongkir);
